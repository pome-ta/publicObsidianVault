...
- `vscode-jsonrpc`
	...
	- 「あなたの」過去の発言メモ
		...
		- `MessageReader` / `MessageWriter` をブラウザAPIで再実装
...
- `@typescript/ata`
	...
	- 「あなたの」過去の発言メモ
		- 自動型取得(Automatic Type Acquisition) `import "p5"` のような外部型定義を自動DL。通信環境が必要
...
`worker-lsp-multi-file.test` がv0.0.1.3 でも、v0.0.2.6 でも沼ったのでメモ
...
- createEnvironment が再実行され、rootFiles が `/file1.ts` 1つで Program 構築
...
- createEnvironment が **2ファイル構成の Program を完全に再構築**
