URL のクエリでコード表現すればいい感じになるのでは？
と、思ったけど微妙かも。

glsl のシェーダーコードの文字列扱いと、
p5js の呼び出し関数だと、
勝手が違うかも。

[jp.wgld.org/js4kintro/editor](http://jp.wgld.org/js4kintro/editor/)

[jp.wgld.org/js4kintro/editor/script.js](http://jp.wgld.org/js4kintro/editor/script.js)
...
editor.getSession().on('change', function(e){
    ...
    bid('htmlCount').textContent = editor.getValue().length + 2855;
    });
...
function setShortenUri(uri){
  var e = bid('uri');
  if(uri.error){
    e.value = uri.error.message;
  }else{
    bid('uri').value = uri.id;
  }
}
...
function shorten(){
  ...
  xhr.addEventListener("load", function(){setShortenUri(JSON.parse(xhr.response));}, true);
  xhr.open("POST", "https://www.googleapis.com/urlshortener/v1/url?key=AIzaSyCDMxCiUTDH7orIpm_53eNgyAFyDO66ubM");
