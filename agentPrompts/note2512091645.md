...
以下の状態を最新版v0.0.3.3 として、記録して。
参照すべきコードは、最新版構成を固定。

v0.0.3.3 は、テストコードで示すと、`vfs-openFile.test.js` であり、テストは通過（グリーン）している。
...
`v{a}.{b}.{c}.{d}` （それぞれの位を `a`, `b`, `c`, `d` ）とすると
...
と、スモールステップで数値をインクリメントしていく。

`d` で、（現状の）`lsp-core.js` や`worker.js` など、テストではない機能的なコードを書き換えることもあるが、機能的なコードは極力モジュール分割は行わない方針。
理想は、`c`  が変わるタイミングで、分割を検討。

TDD的（「テスト」については後述）な流れで、
機能的なコードを書き換えることは、大いに可能性はある。
...
の順番で、レッドシグナルからグリーンシグナルを目指す。

実行結果のlog を私があなたに提示し、挙動の確認を行う。

#### テストのための`test-utils.js`

`test-utils.js` もバージョンごとに準備。
私のブラウザ実行の結果のlog を出す機能などを実装している。

※ webWorker のlog が、私の実行環境では簡単に出ないため、DOM にて出せるようにしている


### 実行環境

基本的にiOS safari で実行。log は、eruda を経由。
...
1. VfsCore: multi-file read テスト強化
    - openFile、updateFile、getFileSnapshot の一貫性検証
2. VfsCore: incremental update テスト開始
    - 特に TS incremental program までは行かず、メモリ上の TS VFS の増減をテストするフェーズ
3. LspServerCore との連結テスト準備
    - VFS の準備が整ったので、LSP 側と結合する土台づくり
4. worker.js の RPC の整理
    - vfs/* 系 RPC の命名と実装の軽量化
