## 実行結果 

> ① VFSに本当に入っているか

`true` を返す

> ② TypeScript Programに入っているか

`file:///p5.sound.d.ts` 確認できる


## 違う方向の調査

### p5.js で、

p5.sound の`.d.ts` 挙動確認の前提基本として、p5.js の挙動を確認した。

現在は、`ATA` で取得をしてしまうので、コードで`ATA` 挙動を全てOFF にした。


大前提として、DefinitelyTyped の`@types/p5` は、v2系では、使用していない。*使わない* のである。

蛇足だが、`ATA` は、私のコード上で`this.#ata("import 'p5';");` をすると、
（あなたとは違い、正しく）`https://cdn.jsdelivr.net/npm/p5@2.3.0/types/` と、v2系から、`.d.ts` を取得していることが、確認できる。

整理すると、v2系のp5.js のディレクトリ（型情報としての）構成は以下:

```
p5/
  ├ package.json
  └ types/
    ├ global.d.ts
    └ p5.d.ts
  
```

私は、上記構成をローカルに順番して
`#init` 内で、`fetch` で取得して`this.createVirtualFile` して手動で登録することにした。

- `this.createVirtualFile('file:///node_modules/p5/package.json', json文字列)`
- `this.createVirtualFile('file:///node_modules/p5/types/global.d.ts', global文字列)`
- `this.createVirtualFile('file:///node_modules/p5/types/p5.d.ts', p5文字列)`

3ファイルを全て、登録した場合には、いままでの挙動通りとなった。
しかし、`package.json` を登録しない場合には、補完が出なくなった。


```package.json

  "name": "p5",
  "repository": "processing/p5.js",
  "type": "module",
  "scripts": {
    "build": "rollup -c",
    "dev": "vite preview/",
    "dev:global": "concurrently -n build,server \"rollup -c -w\" \"npx vite preview/global/\"",
    "docs": "documentation build ./src/**/*.js ./src/**/**/*.js --shallow -o ./docs/data.json && node ./utils/convert.mjs",
    "bench": "vitest bench",
    "bench:report": "vitest bench --reporter=verbose",
    "test": "vitest",
    "test:types": "tsc test/types/*.ts --noEmit --lib es2022,dom",
    "lint": "eslint .",
    "lint:fix": "eslint --fix .",
    "generate-types": "npm run docs && node utils/typescript.mjs"
  },
  "lint-staged": {
    "src/**/*.js": "eslint",
    "test/**/*.js": "eslint",
    "utils/**/*.{js,mjs}": "eslint"
  },
  "version": "2.3.0",
  "dependencies": {
    "@davepagurek/bezier-path": "^0.0.7",
    "@japont/unicode-range": "^1.0.0",
    "acorn": "^8.15.0",
    "acorn-walk": "^8.3.4",
    "colorjs.io": "^0.6.0",
    "escodegen": "^2.1.0",
    "gifenc": "^1.0.3",
    "i18next": "^19.0.2",
    "i18next-browser-languagedetector": "^4.0.1",
    "libtess": "^1.2.2",
    "omggif": "^1.0.10",
    "pako": "^2.1.0",
    "zod": "^4.2.1"
  },
  "devDependencies": {
    "@eslint/compat": "^1.2.9",
    "@eslint/js": "^9.28.0",
    "@eslint/markdown": "^6.5.0",
    "@rollup/plugin-alias": "^5.1.1",
    "@rollup/plugin-commonjs": "^25.0.7",
    "@rollup/plugin-json": "^6.1.0",
    "@rollup/plugin-node-resolve": "^15.2.3",
    "@rollup/plugin-replace": "^5.0.7",
    "@rollup/plugin-terser": "^0.4.4",
    "@stylistic/eslint-plugin": "^4.4.1",
    "@vitest/browser": "^4.0.16",
    "@vitest/browser-webdriverio": "^4.0.16",
    "concurrently": "^8.2.2",
    "documentation": "^14.0.3",
    "eslint": "^9.28.0",
    "eslint-plugin-jsdoc": "^50.8.0",
    "glob": "^11.0.1",
    "globals": "^16.2.0",
    "husky": "^4.2.3",
    "lint-staged": "^15.1.0",
    "msw": "^2.6.3",
    "pixelmatch": "^7.1.0",
    "pkg-pr-new": "^0.0.65",
    "rollup": "^4.9.6",
    "rollup-plugin-string": "^3.0.0",
    "rollup-plugin-visualizer": "^5.12.0",
    "typescript": "^5.9.2",
    "vite": "^7.3.0",
    "vite-plugin-string": "^1.2.2",
    "vitest": "^4.0.16",
    "webdriverio": "^9.0.7"
  },
  "license": "LGPL-2.1",
  "browser": "./lib/p5.min.js",
  "exports": {
    ".": {
      "types": "./types/p5.d.ts",
      "default": "./dist/app.js"
    },
    "./global": {
      "types": "./types/global.d.ts",
      "default": "./dist/app.js"
    },
    "./node": {
      "types": "./types/p5.d.ts",
      "default": "./dist/app.node.js"
    },
    "./core": {
      "default": "./dist/core/main.js"
    },
    "./color": "./dist/color/index.js",
    "./shape": "./dist/shape/index.js",
    "./accessibility": "./dist/accessibility/index.js",
    "./friendlyErrors": "./dist/core/friendlyErrors/index.js",
    "./data": "./dist/data/index.js",
    "./dom": "./dist/dom/index.js",
    "./events": "./dist/events/index.js",
    "./image": "./dist/image/index.js",
    "./io": "./dist/io/index.js",
    "./math": "./dist/math/index.js",
    "./utilities": "./dist/utilities/index.js",
    "./webgl": "./dist/webgl/index.js",
    "./webgpu": "./dist/webgpu/index.js",
    "./type": "./dist/type/index.js"
  },
  "files": [
    "dist/**",
    "license.txt",
    "lib/p5.min.js",
    "lib/p5.js",
    "lib/p5.esm.js",
    "lib/p5.esm.min.js",
    "lib/p5.webgpu.js",
    "lib/p5.webgpu.min.js",
    "lib/p5.webgpu.esm.js",
    "translations/**",
    "types/**"
  ],
  "description": "[![npm version](https://badge.fury.io/js/p5.svg)](https://www.npmjs.com/package/p5)",
  "bugs": {
    "url": "https://github.com/processing/p5.js/issues"
  },
  "homepage": "https://p5js.org",
  "directories": {
    "doc": "docs",
    "test": "test"
  },
  "author": "",
  "husky": {
    "hooks": {}
  },
  "msw": {
    "workerDirectory": [
      "test"
    ]
  },
  "types": "./types/p5.d.ts"
}
```


### p5.sound で、

`package.json` が必要なのかと思い、p5.sound でも、

```
p5.sound/
  ├ package.json
  └ types/
    └ p5.sound.d.ts
```

とし、`package.json` は

```package.json
{
  "name": "p5.sound",
  "version": "0.3.0",
  "types": "./types/p5.sound.d.ts"
}

```

と記載をして、p5.js と同様に、`this.createVirtualFile` で登録を行った。


しかし、補完が出ない。TypeScript Program もあるし、VFS には格納されているのを確認できている。
